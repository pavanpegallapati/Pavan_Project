...
public class GenericFunctions extends TestNGListeners {
	...
	/*
	 * Reusable Method Method purpose:Click any link, check the required page is
	 * opened or not Created By: Pavan Date: 07-June-2018 Reviewed BY: Mohan
	 */

	public static boolean page_navigation_is_successful(WebElement Header, String value) {
		boolean status = true;

		try {
			//
			String str_header = Header.getText();
			System.out.println(str_header);

			if (str_header.contentEquals(value)) {
				status = true;
	...
	public static boolean Text_Header_available(WebElement Header_Text, String value) {
		boolean status = true;

		try {

			String str_header = Header_Text.getAttribute(value);
			System.out.println(str_header);
